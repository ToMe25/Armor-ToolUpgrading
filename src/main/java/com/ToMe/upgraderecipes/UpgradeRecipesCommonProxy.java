...
public class UpgradeRecipesCommonProxy {
	...
	protected static HashMap<String, ResourceLocation> ItemMap = new LinkedHashMap<String, ResourceLocation>();
	...
	public void preInit(FMLPreInitializationEvent e) {
		...
		if(Config.dummyItems) {
			//addDummyItems();
			//TODO UpgradeRecipesMod.proxy.addDummyItems();
		}
	}
	...
	public void Init(FMLInitializationEvent e) {
		jsonRecipes();
		ItemFinder.initTypes();
		UpgradeRecipesMod.cfg.initItemMap();
		initMapsAndLists();
	...
	public void registerItems(RegistryEvent.Register<Item> e) {
		UpgradeRecipesMod.log = LogManager.getLogger(UpgradeRecipesMod.NAME);
		if(UpgradeRecipesMod.cfg == null) {
	...
	//protected void initMaterialMap() {
		...
		//TODO MaterialMap.put("lapis", Config.lapis_use_ingot ? "ingotRefinedLapis" : "gemLapis");
	...
	protected static void initItemMaterialMap() {
		...
		for(String type:Config.items.keySet()) {
			//if(type != "boots") {
				//type = type.substring(0, type.length() - 1);
			//}
			for(String item:Config.items.get(type)) {
				//if(type != "boots") {
				//if(type != "boots" && type.endsWith("s")) {
				//if(!type.equalsIgnoreCase("boots") && type.endsWith("s")) {
				if(!type.equalsIgnoreCase("boots") && !type.equalsIgnoreCase("leggings") && type.endsWith("s")) {
					type = type.substring(0, type.length() - 1);
				}
				ResourceLocation registryName = new ResourceLocation(item);
				ItemMap.put(ItemFinder.getMaterial(registryName, type) + ":" + type, registryName);
			}
		}
